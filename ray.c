...
double update_dir(t_scene *scene, t_player *p)
{
	...
	if (p->angle == 0)
	{
		...
		// p->angle =  fabs(p->angle);
	// if (p->angle < 0)
	// 	return(-p->angle); + p->y_dir
...
int find_next_vert(t_scene *scene, t_ray *ray, int from_x, int from_y)
{
	...
	ray->next_vert_y = (int)(dist_to_next_vert_intersect * fabs(sin(p->angle)) * p->y_dir + (from_y));// * p->y_dir 
...
void calc_ray(t_scene *scene)
{
	...
	printf("angle %f, p->x_dir %d, p->y_dir %d\n", p->angle, p->x_dir, p->y_dir);

	if (ray)
		scene->ray = ray;

	while (is_inside_map(scene, ray->next_vert_x, ray->next_vert_y) &&
		not_wall(scene, ray->next_vert_x, ray->next_vert_y, cv)) //+ SEGM * x->dir +  SEGM * y->dir
	...
	while (is_inside_map(scene, ray->next_hor_x, ray->next_hor_y) &&
		not_wall_hor(scene, ray->next_hor_x, ray->next_hor_y, ch))
		{
